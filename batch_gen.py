#!/usr/bin/python3.6
#adapted from https://github.com/ZheLi2020/TimestampActionSeg
import torch
import numpy as np
import random
...
class BatchGenerator(object):
    def __init__(self, num_classes, actions_dict, gt_path, features_path, sample_rate, jump, threshold, dataset_name):
        ...
        self.random_index = np.load(gt_path + dataset_name + "_annotation_using_authgt_{}_{}.npy".format(self.jump,self.threshold),allow_pickle=True).item()  # using gt auth annots for cooking and salads
        self.random_index_extended = np.load(gt_path + dataset_name + "_annotation_extended_using_authgt_{}_{}.npy".format(self.jump,self.threshold), allow_pickle=True).item()  # using gt auth annots for cooking and salads
    ...
    def generate_confidence_mask(self):
        for vid in self.list_of_examples:
            # if 'mpii_cooking2' in self.features_path:
            #     classes = np.load(self.gt_path + vid + '.npy')
            # else:
            ...
            random_idx = np.asarray(self.random_index[vid], dtype=int)
            if '50salads' in self.dataset_name:
                random_idx = random_idx // 2

            # print(random_idx, num_frames)
            ...
            for j in range(len(random_idx) - 1):
                if random_idx[j + 1] == num_frames:  # Added by Saif
                    continue
                ...
                # t_l = (int(classes[random_idx[j]]), random_idx[j],random_idx[j + 1])
                # t_r =(int(classes[random_idx[j + 1]]), random_idx[j],random_idx[j + 1])
    ...
    def get_single_hoi_random(self, batch_size, max_frames):
        # Generate target for only timestamps. Do not generate pseudo labels at first 30 epochs.
        batch = self.list_of_examples[self.index - batch_size:self.index]
        boundary_target_tensor = torch.ones(len(batch), max_frames, dtype=torch.long) * (-100)
        for b, vid in enumerate(batch):
            single_frame = self.random_index[vid]
            frame_idx_tensor = np.asarray(torch.from_numpy(np.array(single_frame)), dtype=int)

            if '50salads' in self.dataset_name:
                frame_idx_tensor = frame_idx_tensor // 2
    ...
    def get_single_hoi_extended_random(self, batch_size, max_frames):
        # Generate target for only timestamps. Do not generate pseudo labels at first 30 epochs.
        batch = self.list_of_examples[self.index - batch_size:self.index]
        boundary_target_tensor = torch.ones(len(batch), max_frames, dtype=torch.long) * (-100)
        for b, vid in enumerate(batch):
            single_hoi_extended_frame = self.random_index_extended[vid.split('.')[0]]
            single_hoi_extended_frame = single_hoi_extended_frame[::self.sample_rate]
            boundary_target_tensor[b, :len(single_hoi_extended_frame)] = torch.from_numpy(single_hoi_extended_frame)
    ...
    def get_boundary(self, batch_size, pred):
        ...
        for b, vid in enumerate(batch):
            single_idx = np.asarray(self.random_index[vid], dtype=int)
            if '50salads' in self.dataset_name:
                single_idx = single_idx // 2  # for 50salads
            vid_gt = self.gt[vid]
            features = pred[b]
            ...
            # Average two action boundaries for same segment and generate pseudo labels using HOI
            ...
            for i in range(num_bound):
                ...
                # Add new logic: For finding the boundary location using HOI
                if self.random_index_extended[vid.split('.')[0]][middle_bound] != -100:
                    prospectives = self.random_index_extended[vid.split('.')[0]][temp_right:temp_left]
                    non_hois = np.asarray(np.where(prospectives==-100),dtype=int)
                    if len(non_hois[0])>1:
                        new_non_hois = non_hois[0][0]
                        middle_bound = new_non_hois + temp_right
                # End of Add new logic
