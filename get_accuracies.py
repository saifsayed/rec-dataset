...
parser.add_argument('--action', default='predict', help='two options: train or predict')
parser.add_argument('--dataset', default="gtea", help='three dataset: breakfast, 50salads, gtea')
parser.add_argument('--split', default='1')
parser.add_argument('--jump', default=75)
...
features_dim = 2048
# bz = 8
bz = 8  # for cooking defualt for salads and others is 8
if 'cooking2' in args.dataset:
    bz = 2
lr = 0.0005
num_epochs = 70
...
bz_stages = '/{}_{}'.format(args.jump, args.threshold)
...
print('jump:{}, threshold:{}'.format(args.jump, args.threshold))
...
if args.action == "train":
    batch_gen = BatchGenerator(num_classes, actions_dict, gt_path, features_path, sample_rate, args.jump,
                               args.threshold, args.dataset)
...
metric_vals = evaluate(args.dataset, args.split, time_data,results_dir)
metric_vals_all.append(metric_vals)
metric_vals_all = np.asarray(metric_vals_all)
np.save('{}/preds_{}_{}_split_{}.npy'.format(results_dir, args.jump, args.threshold, args.split),metric_vals_all)
