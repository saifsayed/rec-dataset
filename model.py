...
class Trainer:
    ...
    def train(self, save_dir, batch_gen, writer, num_epochs, batch_size, learning_rate, device):
        ...
        print('start epoch of hoi supervision is:', start_hoi_epochs)
        for epoch in range(num_epochs):
            ...
            while batch_gen.has_next():
                ...
                if epoch < start_hoi_epochs:
                    batch_boundary = batch_gen.get_single_hoi_random(batch_size, batch_input.size(-1))
                elif epoch < start_epochs and epoch > start_hoi_epochs:
                    batch_boundary = batch_gen.get_single_hoi_extended_random(batch_size, batch_input.size(-1))
                else:
